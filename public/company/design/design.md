...
版本：2026-09-07。适用范围：官网首页、研究、活动、最近新闻、产品和公司栏目。所有尺寸均为 CSS px，不是截图中的物理像素。
...
| 用途                | 字号                     | 字重 | 行高 | 字间距   |
| ------------------- | ------------------------ | ---- | ---- | -------- |
| 页面主标题 H1       | clamp(40px, 5vw, 64px)   | 500  | 1.05 | -0.045em |
| 模块标题 H2         | clamp(28px, 2.7vw, 36px) | 500  | 1.25 | -0.025em |
| 小标题 H3／卡片标题 | 20px；≤720px 时 18px     | 500  | 1.35 | normal   |
| 默认正文            | 17px                     | 400  | 1.65 | normal   |
| 页面导语            | 20px；≤720px 时 18px     | 400  | 1.5  | normal   |
| 眉题、图注、表注    | 14px；≤720px 时 13px     | 400  | 1.35 | normal   |

clamp 表示字号随屏幕宽度变化，同时有最小值和最大值。不能把主标题理解为所有设备都固定 64px。

| 视口宽度      | H1   | H2      | H3   |
| ------------- | ---- | ------- | ---- |
| 390px 手机    | 40px | 28px    | 18px |
| 793px 平板    | 40px | 28px    | 20px |
| 1320px 桌面   | 64px | 35.64px | 20px |
| 1440px 及以上 | 64px | 36px    | 20px |
...
公司与产品详情采用：眉题 → 页面主标题 → 导语 → 可选动作 → 内容模块 → 页脚。普通公司简介默认不再套用彩色描边、圆角和浅色背景卡片，而是使用统一阅读宽度与低密度分段；卡片只用于图片、交互或确实需要独立边界的内容。长段落拆成有明确主题的模块或卡片。技术介绍应包含架构、技术栈与流程图，并配解释文字；不能只堆技术名称，也不能用部署命令代替产品介绍。
...
背景使用 cover 裁切。标题、描述与可选按钮作为一个整体，默认在画面中水平、垂直居中，文字容器最大宽度 1040px，描述最大宽度 880px。标题与描述间距桌面 24px、手机 20px，描述与可选按钮间距桌面 32px、手机 24px。背景叠加由浅到深的黑色渐变以保证白字可读。
...
| 分类到内容列表             | 32px             | 32px         |
| Hero 标题到描述            | 24px             | 20px         |
| Hero 内容到可选按钮        | 32px             | 24px         |
| 公司模块外层上下间距       | 72–120px         | 72px         |
| 公司模块标题到内容         | 32px             | 28px         |
| 首页活动主图与侧栏         | 24px（桌面双栏） | 转为移动布局 |

公司模块使用统一 section 间距，模块交界处不再叠加重复的大额上下 padding。不要同时给外层 section 和内部容器重复添加同一组垂直间距。
...
文章卡片展示图片、标题与必要元信息，首页文章卡片和活动列表不展示简介。产品说明卡可以保留产品定位。卡片背景、边框和圆角根据组件用途继承已有样式；普通公司简介不使用嵌套卡片；Hero 不沿用卡片圆角。
...
字号基础变量位于 src/styles/sections/21-heading-system.css，最终全局编辑层位于 24-global-editorial.css；宽度变量位于 20-page-width.css；字体与基础颜色位于 01-foundation.css，主题覆盖位于 10-theme.css。新增组件通过类名接入共享规则，避免仅凭 H2 标签就假定已经获得统一视觉样式。
